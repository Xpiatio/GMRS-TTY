...
### 3.5 Rolling RX Spectrometer (Waterfall)
- **Audio tap**: `STTWorker` exposes the raw 16 kHz / 512-sample float32 capture chunks on a Qt signal (`audio_chunk`). The spectrometer subscribes alongside VAD/STT — no second capture pipeline. The tap is emitted before pause/VAD branching so the waterfall keeps scrolling during TX (the operator wants to see breakthrough RX or their own carrier).
- **Ring buffer + FFT worker**: `gmrs_tty/audio/spectrogram.py` owns a thread-safe `ChunkRing` plus pure helpers (`hann_window`, `compute_frame`, `magnitude_to_db`, `bin_range_for_band`). `gmrs_tty/audio/spectro_worker.py` is the `QThread` consumer: 1024-sample Hann-windowed frames at 512-sample hop (50 % overlap, ~32 ms per row at 16 kHz). The window array and ring buffer are precomputed once per worker; per-frame allocation is limited to what `numpy.fft.rfft` itself needs.
- **Load shedding**: if the FFT worker or paint thread falls behind, the ring evicts oldest samples on the next push (rounded up to a hop boundary so the producer/consumer grid stays aligned). The capture loop is never back-pressured — VAD/STT cannot starve.
- **Capture-event overlays**: `STTWorker` also emits `capture_event` strings — `vad_start`, `vad_end`, `squelch_opened`, `squelch_closed` — at the corresponding transitions. The widget renders white markers for VAD events and amber markers for squelch edges; markers age leftward one column per emitted row and drop off the left edge of the image.
- **Widget rendering**: `gmrs_tty/ui/spectrogram_widget.py` owns a `QImage.Format_RGB32` of size `(visible_columns, visible_bins)`. Each new row scrolls the existing pixels one column left (in-place numpy view) and writes the new column at the rightmost edge — cheaper than redrawing from a 2-D array per frame and avoids `pyqtgraph` as a dependency. A flipped vertical axis renders high frequencies at the top per the standard waterfall convention.
- **Color maps**: Viridis (33-stop seed interpolated to a 256-entry uint32 BGRA LUT, perceptually uniform and colorblind-safe; default) and Grayscale (linear ramp, hue-free fallback). Defined in `gmrs_tty/ui/spectro_colormap.py`.
- **Settings**: persisted to `config.json` under `spectrometer` as `{enabled, colormap, freq_range, time_window_s, db_floor, db_ceiling}`. Unknown values fall back to defaults so a hand-edited config can't crash the widget.
- **Accessibility**: `accessibleName`/`accessibleDescription` summarize the current band, window, and color map. The widget emits an `activity_text_changed` signal on every row carrying a screen-reader-friendly summary (`"Strong signal at 1.2 kHz"`, `"No signal."`); `MainWindow` surfaces it in the status bar prefixed `"Waterfall: ..."`. Arrow keys adjust the dB floor / ceiling. The widget is never the only indicator of an event — the chat log remains the non-visual fallback.
