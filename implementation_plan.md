...
## Stage 14: Rolling Audio Spectrometer — ⏳ pending
...
*   **Backend choice:** Branch a read-only copy of the existing RX audio frames (the same source that feeds Silero VAD / faster-whisper) into a ring buffer. Run a windowed FFT (Hann window, 1024-sample frames, 50 % overlap at 16 kHz → ~32 ms hop) via `numpy.fft.rfft` — no new heavy dependency. Render in Qt either via direct `QImage` column-blits or `pyqtgraph.ImageItem`; pick whichever measures cheaper on Raspberry Pi at the chosen frame rate.
*   **Tasks:**
    *   ⏳ Audio tap: add a read-only fan-out from the RX capture loop into a thread-safe ring buffer sized for the visible time window (e.g., 30 s × 16 kHz ÷ hop). The tap must not perturb VAD/STT timing — if the consumer falls behind, drop frames rather than back-pressure the producer.
    *   ⏳ FFT worker on a `QThread`: compute magnitude (and log-magnitude in dB) per frame; emit a `numpy.ndarray` row per hop on a Qt signal. Reuse a single FFT plan and a preallocated output buffer to keep per-frame allocation at zero.
    *   ⏳ Spectrometer widget docked above or below the chat pane with a Settings/View toggle (default off on first launch so existing users aren't surprised by the extra CPU). Provide controls for: dB floor/ceiling (gain), color map (default plus at least one colorblind-safe option — e.g., `viridis`), frequency range (full Nyquist vs. voice band 300–3400 Hz), and time window (10 s / 30 s / 60 s).
    *   ⏳ Visual overlays: dim vertical guides at the VAD on/off transitions and at squelch-open pre-trigger edges so the operator can correlate the spectrogram with the existing transcription pipeline events.
    *   ⏳ Accessibility: this widget is *for* deaf/HoH operators, but it must NOT become the only indicator of an event — the chat log / RX bar already gives a non-visual fallback for hearing operators. Required: keyboard-navigable controls, `accessibleName` / `accessibleDescription` summarizing current settings (range, window, color map), no pixel font-size on labels, and a "describe current activity" status line (e.g., "Strong signal at 1.2 kHz, last 3 s") that screen readers can announce.
    *   ⏳ Performance budget: hold the spectrometer at <10 % CPU on a Raspberry Pi 4 with the default settings. If it overruns, auto-degrade by halving the frame rate before dropping the widget — never starve VAD/STT.
    *   ⏳ Unit-test the FFT helper (correct magnitudes for known sine inputs, windowing correctness, dB conversion edge cases, ring-buffer wrap-around). Headless smoke test that the widget instantiates without a display (skip rendering in CI).
    *   ⏳ Document in `README.md` (toggle location, performance notes, the colorblind-safe color map option) and `technical_spec.md` (the audio-tap point, ring-buffer sizing, FFT parameters).