...
## Stage 12: Quick / Common Messages — ⏳ pending
...
*   **Tasks:**
    *   ⏳ Add a horizontal "Quick Messages" strip on the main dashboard (placement: between the Pending Stations bar and the Tx row, or as a collapsible drawer if vertical space gets tight at 150–200 % font scale). Each preset is a button labeled with the phrase; click sends it through the standard TX pipeline so callsign framing, the 15-minute ID rule, PTT keying, and STT auto-pause all still apply.
    *   ⏳ Seed list of presets shipped in `config.example.json`: `Radio check`, `Loud and clear`, `Standing by`, `Acknowledged`, `Negative copy / say again`, `QSY to channel {N}`, `Clear`, `Monitoring`, `Net check-in`, `Emergency traffic`. Keep the list short by default (≤10) so the strip doesn't overwhelm the UI; users grow it from there.
    *   ⏳ User-editable preset list via a small "Quick Messages" dialog reachable from the Settings menu (Alt+S → Alt+Q). Add / edit / remove / reorder rows; persisted to `config.json` (new `quick_messages` key, ordered list of strings). No new file — keep config surface flat.
    *   ⏳ Respect target selection: a preset transmitted while a contact is selected goes "calling {target}" exactly like a typed message would; with `All` selected it's an open call. Presets that already name a target (e.g., "QSY to channel 22") still get the standard prefix — the preset is the *body*, framing is the framing.
    *   ⏳ Placeholder support for `{N}` style tokens in presets (e.g., `QSY to channel {N}`): when clicked, prompt for the value in a tiny inline popover before transmitting, so the operator doesn't have to maintain 22 separate "QSY to channel X" buttons. Keep the tokenizer dumb — `{name}` style, no expressions.
    *   ⏳ Accessibility: every preset button gets a unique `accessibleName` (the phrase itself), a tooltip describing what it transmits, and is reachable via Tab order. Assign global shortcuts only to the first few (e.g., Alt+1 … Alt+9) since unique mnemonics across user-defined phrases aren't guaranteed; document the convention in `technical_spec.md` §2.5.
    *   ⏳ Unit-test: preset round-trip through config persistence, placeholder substitution, target-prefix composition, and that a preset transmission triggers the same ID-timer reset behavior as a typed message containing a callsign.