...
## Stage 13: Parallel LoRa-Mesh Transmit — ⏳ pending
*   **Goal:** When the operator transmits over GMRS voice, optionally relay the same outgoing text in parallel over a connected LoRa mesh device (Meshtastic, Meshcore, or other LoRa/Halo radios) via USB serial or Bluetooth — giving the deaf/HoH operator a second, text-native channel that reaches the mesh network alongside the on-air voice transmission.
*   **Rationale:** GMRS voice is line-of-sight and ear-only; LoRa mesh is text-native, store-and-forward, and routes around obstacles. Sending both at once means recipients without GMRS receivers (or out of GMRS range) still get the message in original text form, and the deaf/HoH operator gains a written acknowledgement loop independent of any other operator's mic technique. The mesh path also preserves the air-gap promise — LoRa mesh networks operate offline by design.
*   **Backend choice:** Pluggable transport adapter. Meshtastic exposes a documented serial/BLE protobuf API via the `meshtastic` Python library — that's the reference adapter. Meshcore and other firmwares (Reticulum/RNode, BT-paired commercial LoRa/Halo modems) plug in behind the same interface. Treat device type, transport (USB/BT), and channel/destination as config fields so adding a new vendor is a new adapter class, not a fork.
*   **Tasks:**
    *   ⏳ Define a `MeshTransport` abstract interface (`connect` / `disconnect` / `send_text` / `status`) and ship a Meshtastic implementation first via `meshtastic` + `pyserial` / `bleak`. Discover devices by enumerating `/dev/ttyUSB*` / `/dev/ttyACM*` and paired BT peripherals; surface the chosen device in Configuration.
    *   ⏳ TX hook: after `_transmit_text(body)` finishes its GMRS path, fan the same body out to any enabled mesh transports. Failures must surface in chat but never block or delay the voice TX — the radio path stays authoritative. Decide per-transport whether to send the pre-expansion typed text or the post-vernacular-expansion form (typed text is the default — mesh recipients expect TTY shorthand to stay compact).
    *   ⏳ Configuration UI: per-transport enable toggle, device picker, channel/destination field, and a "Send to mesh only" override for text-only situations where no voice TX is desired. WCAG 2.1 AA: keyboard nav, `accessibleName` on every control, no pixel font-size, mnemonics that don't collide with existing menu shortcuts.
    *   ⏳ Optional RX bridge (stretch): surface inbound mesh messages in the same chat log with a distinct prefix (e.g., `[mesh] callsign-or-nodeid:`) so the operator sees one unified conversation across voice and mesh.
    *   ⏳ Unit-test the transport adapter contract with a fake device (round-trip, disconnect mid-send, malformed-response handling, voice TX continues when mesh send fails). Real-device integration tests gated behind an env flag, similar to how Whisper/Ollama tests already gate.
    *   ⏳ Document in `README.md` and `technical_spec.md` that the mesh feature is opt-in, that the operator must comply with the rules of whichever sub-GHz band the device operates in (ISM 902–928 MHz in the US for typical LoRa hardware), and that it's independent of the FCC GMRS license governing the voice side.

## Stage 14: Rolling Audio Spectrometer — ✅ done
*   **Goal:** Render a real-time scrolling spectrogram (waterfall) of the live RX audio in the main window so the deaf/HoH operator can *see* signal activity — squelch breaks, voice formants, carrier whistles, neighbor-channel splatter — at a glance, complementing the existing transcription pane.
*   **Rationale:** A spectrogram makes inaudible-to-the-operator events visible: weak voice in noise, intermod, kerchunks, and the simple fact that someone is on-channel before VAD/Whisper has decided anything. Visual feedback shortens the operator's reaction time to mic-up moments and helps diagnose RX quality (squelch threshold, soundcard gain, antenna problems) without depending on hearing. It pairs naturally with [[stage-3-stt]] — the same audio source feeds both, so there is no second capture pipeline to maintain.
*   **Backend choice:** Branch a read-only copy of the existing RX audio frames (the same source that feeds Silero VAD / faster-whisper) into a ring buffer. Run a windowed FFT (Hann window, 1024-sample frames, 50 % overlap at 16 kHz → ~32 ms hop) via `numpy.fft.rfft` — no new heavy dependency. Renders in Qt via direct `QImage` column-blits (`pyqtgraph` would have pulled in a heavy dep for negligible gain on the Pi).
*   **Tasks:**
    *   ✅ Audio tap: `STTWorker` now emits each captured chunk on a new `audio_chunk` Qt signal; `SpectrogramWorker` pushes into a thread-safe `ChunkRing` (see `gmrs_tty/audio/spectrogram.py`). When the consumer falls behind, the ring evicts oldest samples first — the capture loop is never back-pressured.
    *   ✅ FFT worker on a `QThread`: `gmrs_tty/audio/spectro_worker.py` computes Hann-windowed magnitude + dB per hop and emits one `numpy.ndarray` row on `row_ready`. Window is precomputed; the ring reuses its preallocated buffer across frames.
    *   ✅ Spectrometer widget below the chat pane (`gmrs_tty/ui/spectrogram_widget.py`) with a View-menu toggle (default off; `Ctrl+Shift+W`). Controls: dB floor/ceiling via arrow keys, color map (`Viridis` perceptually-uniform + `Grayscale`), frequency range (voice band 300–3400 Hz vs. full Nyquist), and time window (10 / 30 / 60 s). All choices persist to `config.json` under `spectrometer`.
    *   ✅ Visual overlays: white vertical markers at every VAD on/off transition and amber markers at every squelch-open / squelch-close edge. The STT worker emits `capture_event` and the widget renders them via `mark_event`.
    *   ✅ Accessibility: `accessibleName` / `accessibleDescription` summarize current band, window, and color map; the widget emits an `activity_text_changed` signal on every row, surfaced in the status bar as "Waterfall: Strong signal at 1.2 kHz" so screen readers announce meaningful changes. Arrow keys adjust the dB floor / ceiling so keyboard-only operators can tune contrast without leaving the main window. The widget is *not* the only indicator of an event — the chat log remains the non-visual fallback.
    *   ✅ Performance budget: 1024-sample FFT at 50 % overlap (~32 ms hop) sits well under 10 % CPU on a Pi 4 at default settings. The ring drop-on-overflow path is the load-shed mechanism — if the consumer falls behind, samples are evicted instead of starving VAD/STT.
    *   ✅ Unit-tested in `tests/unit/audio/test_spectrogram.py` (32 cases: Hann symmetry, sine-bin localization, dB floor + edge cases, ring-buffer wrap + hop alignment + overflow), `tests/unit/audio/test_spectro_worker.py` (2 cases: end-to-end QThread round-trip, garbage-chunk safety), `tests/unit/ui/test_spectrogram_widget.py` (22 cases: settings round-trip, row push + activity text, marker aging + drop, colormap swap, freq-range swap, keyboard gain), and `tests/unit/ui/test_main_window_spectrometer.py` (11 cases: View-menu wiring, persistence, default visibility, status-bar surfacing).
    *   ✅ Documented in `README.md` (toggle location, performance notes, the colorblind-safe color map option) and `technical_spec.md` (audio-tap point, ring-buffer sizing, FFT parameters).