...
- **Generate Session Journal…** (Ctrl+J) — sends the current transcript and detected callsigns to Gemini and saves the AI-generated journal entry to `journals/`. Requires a Gemini API key configured in Settings → Configuration. Disabled with an informative prompt if the key is missing or the chat is empty.
