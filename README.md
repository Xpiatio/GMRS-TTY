...
- **Service toggle** (top row) — segmented `GMRS` / `FRS` radio buttons (Alt+G / Alt+F). Switching to FRS immediately disables every callsign-specific surface in the app; switching back to GMRS restores them. The selection persists to `config.json` as `radio_service`. Three icon shortcuts anchor the right edge of the same row, left-to-right: a bold **Q** opens the Quick Messages editor (Settings → Quick Messages), a **person-head icon** opens the Contacts editor (Settings → Contacts / Ctrl+B), and a **cog-wheel icon** (rightmost) opens the Configuration dialog (Settings → Configuration / Ctrl+,). The contacts icon disables in FRS mode alongside the menu action; the Q and cog wheel stay enabled in both modes (Quick Messages drives plain TX, Configuration is service-agnostic).
