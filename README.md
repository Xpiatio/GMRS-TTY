...
- **TX watchdog & abort** — a hard cap on transmission length (Behavior tab → Max TX length, default 60 s; 0 disables) force-stops playback and releases PTT if a message runs long, and a synthesis timeout (default 30 s) abandons a stuck Piper synthesis without ever keying the radio. While a transmission is in progress an **Abort TX** button (Esc) appears in the Transmit row as an operator kill switch.
