...
- **Service toggle** (top row) — segmented `GMRS` / `FRS` radio buttons (Alt+G / Alt+F). Switching to FRS immediately disables every callsign-specific surface in the app; switching back to GMRS restores them. The selection persists to `config.json` as `radio_service`. Four icon shortcuts anchor the right edge of the same row, left-to-right: a **🌙 / ☀️ icon** toggles between light and dark themes (persisted in `config.json` as `dark_mode`, default light — the glyph shows the *destination* state, so a moon means "click for dark"; the toggle repaints the header, pending-station pills, chat log, and online indicator together), a bold **Q** opens the Quick Messages editor (Settings → Quick Messages), a **person-head icon** opens the Contacts editor (Settings → Contacts / Ctrl+B), and a **cog-wheel icon** opens the Configuration dialog (Settings → Configuration / Ctrl+,). Finally, the **● Online / ○ Offline indicator** sits at the far right of the same row (relocated from the status bar) so connectivity state is visible alongside the action icons; it hides in FRS mode where FCC lookups don't apply. The contacts icon disables in FRS mode alongside the menu action; the theme toggle, Q, and cog wheel stay enabled in both modes (theme is service-agnostic, Quick Messages drives plain TX, Configuration is service-agnostic).
