...
- **Noise reduction** (spectral gating) applied per utterance after bandpass, followed by a configurable **gain stage** so weak or distant stations reach Whisper at a consistent level. Three modes in Settings → Configuration → STT → Gain mode: **Dynamic AGC** (default — fast-attack/slow-release levelling across the utterance), **RMS normalize** (one flat gain to −20 dBFS), or **off**.
- **Noise-profile denoising** (Settings → Configuration → STT → Noise profile, default off) — while the squelch is closed the channel is pure noise floor; the app samples that static and uses it as the denoiser's stationary noise estimate instead of guessing from the speech itself. Can improve accuracy on consistently noisy channels.
- Offline transcription via **faster-whisper** (`small.en` by default, int8 CPU; pick any staged model in Settings → Configuration → STT → Whisper Model). Decoding uses beam size 5 with per-segment confidence filtering, so low-confidence noise segments are dropped instead of appearing as gibberish.
- **Vocabulary biasing** — Whisper's initial prompt is assembled from built-in radio procedure words (NATO alphabet, Q-codes, prowords), your **custom phrases** (Settings → Configuration → STT → Custom phrases: names, landmarks, club jargon, one per line), and your saved contacts' callsigns (newest first, capped by **Max callsigns** — set it to 0 to leave callsigns out entirely). The bias list updates live when contacts change — a newly saved callsign is recognized without restarting Listen.
- Drops short blips (<400 ms) and common Whisper hallucinations on silence.
- **Utterances finalize on carrier drop** — when the squelch closes mid-speech the transmission is over; the app trims the static crash from the tail and finalizes immediately instead of waiting for the VAD to time out.
- **Two-tier transcription (optional second pass)** — Settings → Configuration → STT → Final-pass model (default **Off**). When enabled, the streaming model keeps delivering live partials as before, and the moment a transmission ends the *whole* utterance is re-transcribed by a larger model (`large-v3-turbo` recommended; **Auto** picks the best model staged under `Models/STT`) on a low-priority background thread — the chat line is then upgraded in place with the more accurate text. Utterances longer than **Final-pass max length** (default 60 s) keep their streaming transcript rather than risk a truncated re-read. Stage a final model with `python bootstrap_models.py --model small.en large-v3-turbo`.
- **Optional GPU acceleration for the final pass** — with a supported AMD (ROCm) or NVIDIA (CUDA) GPU, install the extras in `requirements-gpu.txt` (torch with the matching index URL, plus transformers) and stage the HF-format model with `python bootstrap_models.py --final-model large-v3-turbo --final-backend gpu`. Set **Final-pass device** to Auto or GPU; any GPU failure falls back to CPU automatically. *Not required for normal operation and never installed by the .deb — the base app stays CPU-only.*
- **STT calibration wizard** (Tools → Calibrate STT, enabled while Listen is active) — a four-step guided tuner: it shows a reference passage, records it being read over the air, then sweeps every staged Whisper model × gain mode × noise profile against the reading and ranks the combinations by word-error-rate. **Apply selected** writes the winning model/gain/noise-profile settings to the configuration (a model change takes effect the next time Listen starts). A full sweep on CPU takes minutes when multiple models are staged — the progress bar shows "combination N of M" throughout, and Escape cancels cleanly at any step.
- **STT debug capture & offline evaluation** (Settings → Configuration → STT → Debug capture, default off) — records every utterance's raw, segmented, and processed audio plus all transcripts to the debug directory (`debug/stt` by default). Hand-label captures with a `reference.txt` and replay them through the exact production pipeline with `python -m gmrs_tty.tools.eval_stt --audio debug/stt --model small.en` to measure word-error-rate; flags let you A/B denoise, gain mode, noise profile, decode options, and more (`--json` for machine-readable output).
...
- **VOX priming** (PTT tab, default off) — for VOX-keyed radios whose attack clips the first word: a **primer tone** (1 kHz burst, 50–2000 ms, then a short settle gap) keys the radio before speech starts, and/or a spoken **priming word** (default "transmit") prefixes the message so the radio keys on a clear keyword.
- **TX audio conditioning** (Behavior tab, default off) — band-limits synthesized speech to the 300–3000 Hz FM voice channel, applies gentle downward compression, normalizes the voiced level to −18 dBFS, and caps peaks at −1 dBFS, so the voice modulates the radio consistently without clipping or splatter. Conditioning runs before the lead-in/tail padding is spliced in, so the silence around the keying stays exact zeros. Leave off if TTS plays through regular speakers.
- **TX watchdog & abort** — a hard cap on transmission length (Behavior tab → Max TX length, default 60 s; 0 disables). A message that is already over the cap is cancelled before the radio is keyed, so you get a "shorten it" notice instead of a transmission chopped off mid-word; playback that still overruns is force-stopped with PTT released. A synthesis timeout (default 30 s) abandons a stuck Piper synthesis without ever keying the radio. While a transmission is in progress an **Abort TX** button (Esc) appears in the Transmit row as an operator kill switch.
...
- **Net session history & attendance stats** — the panel's **Save session** button (or the config-gated auto-save: Settings → Configuration → Behavior → Auto-save sessions, which stores the grid every time Listen stops, skipping empty sessions) writes the current roster to a timestamped JSON record under `net_sessions/`. **Tools → Net Attendance History** opens a two-tab browser: **History** lists every saved session newest-first with its full roster, per-session CSV export, an export-all CSV (one row per station per session), and per-session delete (path-traversal-guarded); **Statistics** aggregates per-station attendance — total nets, attendance over the last 10, current streak, last seen — sorted busiest-first, with its own CSV export. A station is a callsign + name pair, so family members sharing one GMRS callsign count separately. The panel also has an **Export CSV** button for the live grid.
