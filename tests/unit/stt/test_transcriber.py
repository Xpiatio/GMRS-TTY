...
@pytest.fixture
def stub_whisper_model(monkeypatch):
    """Replace faster_whisper.WhisperModel with a MagicMock so load() never
    touches CTranslate2 or the filesystem (the real package is installed in
    this environment, unlike Radio-TTY's CI which stubbed the module)."""
...
class TestLoadCpuThreads:
    def test_default_leaves_one_core_free(self, stub_whisper_model):
        import os
        WhisperTranscriber.load("/tmp/model")
        expected = max(1, (os.cpu_count() or 2) - 1)
        assert stub_whisper_model.call_args.kwargs["cpu_threads"] == expected

    def test_explicit_cpu_threads_forwarded(self, stub_whisper_model):
        WhisperTranscriber.load("/tmp/model", cpu_threads=2)
        assert stub_whisper_model.call_args.kwargs["cpu_threads"] == 2

    def test_decode_options_and_word_confidence_min_forwarded(self, stub_whisper_model):
...
class TestPromptStyle:
    ...
    def test_load_forwards_prompt_style(self, stub_whisper_model):
        t = WhisperTranscriber.load(
