...
# torch/transformers are optional (requirements-gpu.txt) and absent from the
# base environment. Stub only what is genuinely missing so load() paths are
# testable; a real installation is left untouched.
for _name in ("torch", "transformers"):
    if importlib.util.find_spec(_name) is None:
        sys.modules.setdefault(_name, MagicMock())

from gmrs_tty.stt.gpu_transcriber import GpuWhisperTranscriber  # noqa: E402
