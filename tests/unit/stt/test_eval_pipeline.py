"""Tests for gmrs_tty.tools.eval_stt — offline pipeline replay + WER scoring.
...
from gmrs_tty.tools.eval_stt import (
    EvalPipelineConfig,
...
def test_eval_config_gain_mode_defaults_to_agc():
    from gmrs_tty.tools.eval_stt import EvalPipelineConfig
...
def test_stage_toggles_forwarded(monkeypatch):
    seen = {}
    import gmrs_tty.tools.eval_stt as mod
...
def _patch_main_deps(monkeypatch, mod, *, decode_calls=None, vad_calls=None, hypothesis="hello world"):
    ...
    monkeypatch.setattr("gmrs_tty.audio.vad.load_vad_model", lambda: object())
    monkeypatch.setattr("gmrs_tty.audio.vad.make_vad_iterator", fake_make_vad_iterator)
...
def test_load_noise_override_reads_sibling_noise_wav(tmp_path):
    ...
    from gmrs_tty.tools.eval_stt import _load_noise_override
